...
print(factor)
    # DELCN=np.append(DELCN,DCOC)
    # DELSEC=np.append(DELSEC,secarea)
    ...
    # Dataframe = pd.DataFrame(Displace)
    # model = sk.pipeline.make_pipeline(sk.preprocessing.PolynomialFeatures(4),sk.linear_model.Ridge(alpha=7.75,solver='cholesky'))
    # model.fit(Dataframe[['X_coord','Y_coord']],Dataframe.Z_coord)
    # #C= model._final_estimator.coef_
    
    # x0, x1 = np.meshgrid(np.linspace(-.5,.5,2001),np.linspace(-.5,.5,2001))
    # XY=np.vstack((x0.reshape(-1),x1.reshape(-1))).T
    
    # Zm = model.predict(XY)+model._final_estimator.intercept_
    # Zm = Zm.reshape(x0.shape)
    
    #%% plot
    
    # plt.figure(figsize=(3,3))
    # plt.grid(1)
    # plt.contourf(x0,x1,Zmod0,cmap='Pastel2')
    # plt.xlabel("X")
    # plt.ylabel("Y")
    ...
    # surf = ax.plot_surface(x0, x1, Zmod, cmap='magma',linewidth=4, antialiased=True)
    # ax.set_zlim(0, .5)
    ...
    # secarea = 0
    # cellastor = 0
    # Areatot = ((x0[-1,-1]-x0[0,0])**2)*((x1[-1,-1]-x1[0,0])**2)
    # le = np.shape(Zm)
    # for i in range(0,le[0]-1):
    #     for j in range(0,le[1]-1):
            
    #         X =  abs(x0[i,j]);   X1 = abs(x0[i,j+1]);   DX = abs(X-X1)
    #         Y =  abs(x1[i,j]);   Y1 = abs(x1[i+1,j]);   DY = abs(Y-Y1)            
    #         Z  = abs(Zm[i,j]);   ZX = abs(abs(Zm[i+1,j])-Z);   ZY = abs(abs(Zm[i,j+1])-Z);   ZXY = abs(abs(Zm[i+1,j+1])-Z)
            
    #         p1 = [ -DY*ZX , -(DX*ZXY-DX*ZX) , DX*DY]
    #         p2 = [ DY*ZXY-DY*ZY , -(-DX*ZY) , -DX*DY]
            
    #         mu1 = np.sqrt(p1[0]**2+p1[1]**2+p1[2]**2)/2
    #         mu2 = np.sqrt(p2[0]**2+p2[1]**2+p2[2]**2)/2
    #         cellarea = mu1+mu2
    #         cellastor = cellastor + DX*DY*(((cellarea)**2)/((DX*DY)**2)-1) 
            
    #         secarea = secarea + cellarea - DX*DY
    # DCOC = cellastor/Areatot
