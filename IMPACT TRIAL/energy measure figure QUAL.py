...
e1=np.asarray([[85.4264,85.4306,85.4408],
               [70.9824,70.987,70.9976],
               [85.1267,85.1307,85.1413],
               [112.34,112.34,112.354],
               [119.016,119.016,119.032],
               [86.32,70.9872,86.3316],
               [48.4644,48.4696,48.4791],
               [63.91,112.345,63.925],
               [81.2746,81.2799,81.2896],
               [1.3255,1.3307,1.4407],#11split
               [48.4644,48.4696,48.4791],
               [63.9101,63.9153,63.9252],
               [81.2746,81.2799,81.2896],
               [122.83,122.835,122.844],
               [1.016,1.021,1.032],#16split
               [96.1265,96.1316,96.1412],
               [1.016,1.021,1.032],#18split
               [1.016,1.021,1.032],#19split
               [1.016,1.021,1.032],#20split
               [1.016,1.021,1.032],#21split
               [126.971,126.978,126.987],
               [1.016,1.021,1.032],#23split
               [106.084,106.089,106.1],
               [140.033,140.039,140.049],
               [86.829,83.8346,86.8435],#27split
               [1.016,1.021,1.032],#28spilt
               [1.016,1.021,1.032],#29split
               [132.304,132.309,132.317],
               [1.016,1.021,1.032],#31split
               [1.016,1.021,1.032]])#32split
E=[]
E1=[]
E0=[]
ra= np.arange(0,len(LOAD))
for file in np.arange(0,9):
    ...
    inforce,intt=strip_plot(acc,tt,e1[file,0],e1[file,2])
...
plt.contourf(x0,x1,Cbre,cmap='coolwarm',levels=800,vmin=0,vmax=1.2)
...
fig, axs = plt.subplots(nrows=3, ncols=len([0,1,3,4,6,7]),figsize=(14,6))
a=0
for i in [0,1,3,4,6,7]:
    ...
    axs[1,a].plot(tt,cc,"-"); axs[1,a].axis(ymin=0.999*np.min(cc1),ymax=1.001*np.max(cc2));
    axs[1,a].plot(tt1,cc1,"-",c='tab:green'); axs[1,a].plot(tt2,cc2,"-",c='tab:orange');
    axs[2,a].plot(tt1,np.zeros(np.shape(cc1)),"-",c='tab:green'); axs[2,a].plot(tt2,dcav2,"-",c='tab:orange');
    axs[2,a].axis(ymin=-1/1000,ymax=5/1000)
    
    inf=ED.iloc[i,0];  #inf1=ED.iloc[i,1]; inf2=ED.iloc[i,2];
    ttf=ED.iloc[i,1];  ttf= np.linspace(ttf[0],ttf[-1],num=len(inf))-ttf[0];
    #ttf1=ED.iloc[i,4]; ttf1=np.linspace(ttf1[0],ttf1[-1],num=len(inf1))-ttf1[0];
    #ttf2=ED.iloc[i,5]; ttf2=np.linspace(ttf2[0],ttf2[-1],num=len(inf2))-ttf2[0];
    
    #bo1=inf[0]*np.ones(np.shape(inf1)); 
    
    axs[0,a].plot(ttf,inf,"-");
    #axs[0,i].set_title(str(str(round(E[i],3))+'J'))
    axs[0,a].axis(ymin=0.95*np.min(inf),ymax=1.05*np.max(inf));
    a=a+1
    #axs[0,i].plot(ttf1,inf1,"-",c='tab:orange'); 
    #axs[0,i].fill_between(ttf1, bo1, inf1, color="tab:orange", alpha=.5)
    #axs[0,i].scatter(ttf1[-1],inf1[-1],color="tab:orange")
